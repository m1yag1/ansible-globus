...
@pytest.mark.high_assurance
def test_auth_project_deletion(
    ansible_playbook_auth_params_auth, create_playbook, run_playbook
):
    """Test project deletion (requires high-assurance auth).

    This test requires recent MFA authentication and will fail in CI.
    Run locally with: pytest -m high_assurance
    """
    playbook_content = f"""
---
- hosts: localhost
  ...
  gather_facts: true
  tasks:
    - name: Create test project for deletion
      community.globus.globus_auth:
        resource_type: project
        name: "ansible-deletion-test-{{{{ ansible_date_time.epoch }}}}"
        contact_email: "test@example.com"
        description: "Test project for deletion testing"
        admin_ids:
          - "{{{{ lookup('env', 'GLOBUS_ADMIN_IDS') }}}}"
        {ansible_playbook_auth_params_auth}
        state: present
    ...
    - name: Verify project was deleted
      assert:
        that:
          - delete_result.changed
"""
...
@pytest.mark.high_assurance
def test_auth_client_deletion(
    ansible_playbook_auth_params_auth, create_playbook, run_playbook
):
    """Test client deletion (requires high-assurance auth).
...
- hosts: localhost
  ...
  tasks:
    ...
    - name: Create test client for deletion
      community.globus.globus_auth:
        resource_type: client
        name: "test-deletion-{{{{ ansible_date_time.epoch }}}}"
        project_id: "{{{{ project_result.resource_id }}}}"
        client_type: confidential_client
        {ansible_playbook_auth_params_auth}
        state: present
    ...
    - name: Verify client was deleted
      assert:
        that:
          - delete_result.changed
"""
